...
public class AbstractV1_8_9 implements AbstractVersion {
    ...
    public boolean isChunkLoaded(Location location) {
        ...
        val blockPos = new BlockPosition(location.getBlockX(), location.getBlockY(), location.getBlockZ());
        return !nmsWorld.isLoaded(blockPos)
                && nmsWorld.getChunkAtWorldCoords(
                        new BlockPosition(location.getBlockX(), 0, location.getBlockZ())).o();
    }

    @Override
    public float getMovementFactor(LivingEntity entity) {
        return ((CraftLivingEntity)entity).getHandle().bI();
    }
