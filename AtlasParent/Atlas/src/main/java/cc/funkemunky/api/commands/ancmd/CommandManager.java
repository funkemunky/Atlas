...
public class CommandManager implements CommandExecutor {
    ...
    public void registerCommands(Object clazz) {
        try {
            ...
            Arrays.stream(clazz.getClass().getMethods())
                    .filter(method -> method.isAnnotationPresent(Command.class)
                            && method.getParameterCount() > 0
                            && method.getParameters()[0].getType() == CommandAdapter.class)
                    .forEach(method -> {
    ...
    public void unregisterCommands(Plugin plugin) {
        commands.keySet()
                .stream()
                .filter(key -> commands.get(key).getPlugin().getName().equals(plugin.getName()))
                .forEach(key -> {
        ...
        try {
            ...
            for (String alias : cmd.getAliases()) {
                if (knownCommands.containsKey(alias)
                        && knownCommands.get(alias).toString().contains(Atlas.getInstance().getName())) {
        ...
        for (int arg = args.length; arg >= 0; arg--) {
            ...
            if (commands.containsKey(buffer.toString())) {
                ...
                if (command.playerOnly() && !(sender instanceof Player)) {
                    ...
                    String[] modArgs = IntStream
                            .range(0, args.length - subCommand).mapToObj(i -> args[i + subCommand])
                            .toArray(String[]::new);
                    ...
                    if (command.permission().length == 0
                            || Arrays.stream(command.permission()).anyMatch(sender::hasPermission)) {
                        CommandAdapter adapter = new CommandAdapter(
                                sender,
                                cmd,
                                sender instanceof Player ? (Player) sender : null, labelFinal, command, modArgs);
                        try {
    ...
    public ColorScheme getDefaultScheme() {
        return new ColorScheme(Color.Gold + Color.Bold, Color.Gray, Color.Yellow,
                Color.White, Color.Dark_Gray, Color.Red, Color.White);
    }
    ...
    public void runHelpMessage(CommandAdapter command, CommandSender sender, ColorScheme scheme) {
        try {
            ...
            sender.sendMessage(scheme.getTitle() + command.getAnnotation().display()
                    + scheme.getBody() + " Help " + scheme.getValue()
                    + "Page (" + page + " / " + (int) MathUtils.round(arguments.size() / 6D) + ")");
            sender.sendMessage("");
            sender.sendMessage(Color.translate(scheme.getBody2nd()) + "<> " + scheme.getBody()
                    + "= required. " + scheme.getBody2nd() + " [] " + scheme.getBody() + "= optional.");
            sender.sendMessage("");
            if (sender instanceof Player) {
                ...
                if (cmdaliases.size() > 0) {
                    for (String aliase : cmdaliases) {
                        cmdaliasesFormatted.append(scheme.getValue())
                                .append(aliase).append(scheme.getBody()).append(", ");
                    }
                ...
                for (int i = (page - 1) * 6; i < Math.min(page * 6, arguments.size()); i++) {
                    ...
                    if (aliases.size() > 0) {
                        for (String aliase : aliases) {
                            aliasesFormatted.append(scheme.getValue())
                                    .append(aliase).append(scheme.getBody()).append(", ");
                        }
                    ...
                    String hoverText = Color.translate((argument.permission().length > 0
                            ? scheme.getTitle() + "Permissions: " + scheme.getValue()
                ...
                for (int i = (page - 1) * 6; i < Math.min(arguments.size(), page * 6); i++) {
                    Command argument = arguments.get(i);
                    sender.sendMessage(scheme.getBody() + "/" + command.getLabel().toLowerCase()
                            + scheme.getValue() + " " + (argument.display().length() > 0
                            ? argument.display()
                            : argument.name()) + scheme.getBody() + " to " + argument.description());
                }
            }
