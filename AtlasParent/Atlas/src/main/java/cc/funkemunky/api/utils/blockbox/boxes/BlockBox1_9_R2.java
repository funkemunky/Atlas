...
public class BlockBox1_9_R2 implements BlockBox {
    ...
    public CollisionBox getCollisionBox(Block block) {
        final net.minecraft.server.v1_9_R2.World world =
                ((org.bukkit.craftbukkit.v1_9_R2.CraftWorld) block.getWorld()).getHandle();
        final int x = block.getX(), y = block.getY(), z = block.getZ();
        final AxisAlignedBB collide = BlockBoxManager.cbox.copy().offset(x, y, z).toAxisAlignedBB();

        List<AxisAlignedBB> boxes = new ArrayList<>();

        net.minecraft.server.v1_9_R2.Block vblock = CraftMagicNumbers.getBlock(block);
